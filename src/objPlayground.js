...
const calculator = (number1, number2) => {
  const sumVariavel = number1 + number2;
  const multVariavel = number1 * number2;
  const divVariavel = Math.floor(number1 / number2);
  const subVariavel = number1 - number2;
  return {
    sum: sumVariavel,
    mult: multVariavel,
    div: divVariavel,
